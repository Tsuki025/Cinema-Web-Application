        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('grafik.update', $grafik->GrafikID) }}">
                        @csrf
                        @method('PUT')

                        <label for="data" class="block font-medium text-sm text-gray-700">Data:</label>
                        <input type="date" id="data" name="data" value="{{ $grafik->data }}" class="border rounded p-2 w-full mb-4">
                        ...
                        <select id="PracownikID" name="PracownikID" class="border rounded p-2 w-full mb-4">
                            ...
                            <option value="{{ $pracownik->PracownikID }}" @if($grafik->PracownikID == $pracownik->PracownikID) selected @endif>
                        ...
                        <select id="zmiana" name="zmiana" class="border rounded p-2 w-full mb-4">
                            <option value="1" @if($grafik->GodzinaOd == '08:00') selected @endif>Zmiana 1 (08:00 - 16:00)</option>
                            <option value="2" @if($grafik->GodzinaOd == '16:00') selected @endif>Zmiana 2 (16:00 - 00:00)</option>
                        </select>

                        <div class="flex justify-between">
                            <button type="submit" class="bg-green-500 text-white px-4 py-2 rounded">Zapisz zmiany</button>
                            <a href="{{ route('grafik.index', ['data' => $grafik->data]) }}" class="bg-red-500 text-white px-4 py-2 rounded">Anuluj</a>
                        </div>
                    </form>
